/*!
        @file    test_Spectrum_Staggered_2ptFunction_eo_WallSource.cpp
...
/*!
    Staggered fermion spectroscopy with even-odd version of Fopr.
    ...
    - Normalization of Fopr_Staggered_eo is changed from hopping
      to mass normalization. Fprop_Standard_eo is adopted.
                                            [21 Nov 2021 H.Matsufuru]
 */
...
//====================================================================
  int hadron_2ptFunction_eo_wallSource(void)
  {
    ...
    fopr_type += "_eo";
    vout.general(vl, "fermion_type = %s\n", fopr_type.c_str());
    unique_ptr<Fopr> fopr_eo(Fopr::New(fopr_type, params_fopr));
#pragma omp parallel
    {
    ...
    unique_ptr<Solver> solver(Solver::New(solver_type,
                                          fopr_eo.get(), params_solver));

    unique_ptr<Fprop_Standard_eo> fprop_eo(
      new Fprop_Standard_eo(solver.get()));

    unique_ptr<Source_Staggered_Wall> source(new Source_Staggered_Wall(params_source));

    vout.general(vl, "preparation finished.\n");
    ...
    for (int isrc = 0; isrc < 2; ++isrc) {
      for (int ic = 0; ic < Nc; ++ic) {
        ...
        int    nconv;
        double diff;
        fprop_eo->invert_D(sq[icd], src, nconv, diff);
