...
/*!
    ...
    Adopt new parameter handlings.        [26 Jun 2016 T.Aoyama]
...
namespace Test_Spectrum {
  ...
  {
    return hadron_2ptFunction("test_Spectrum_Wilson_TwistedMass_Hadron2ptFunction.yaml");
  }
  ...
  namespace {
    ...
    static const bool is_registered_Wilson_TwistedMass = TestManager::RegisterTest(
      "Spectrum.Wilson_TwistedMass.Hadron2ptFunction",
      hadron_2ptFunction_Wilson_TwistedMass
      );
    ...
    for (int iq = 0; iq < N_quark; ++iq) {
      ...
      for (int ispin = 0; ispin < Nd; ++ispin) {
        for (int icolor = 0; icolor < Nc; ++icolor) {
          ...
          int    Nconv;
          double diff;
          quark[iq].fprop->invert_D(sq[iq][i_cd], b, Nconv, diff);
