/*!
        @file    test_HMC_Wilson_Nf2_TopologyFixing.cpp
...
/*!
    This class tests HMC update for dynamical Wilson fermion with
    smeared topology fixing term.         [24 Dec 2011 H.Matsufuru]
...
namespace Test_HMC_Wilson {
  const std::string test_name = "HMC.Wilson.Nf2_TopologyFixing";

  //- test-private parameters
  namespace {
    const std::string filename_input = "test_HMC_Wilson_Nf2_TopologyFixing.yaml";
  }
  ...
  int update_Nf2_topology_fixing(void)
  {
    ...
    const Parameters params_fopr_prec  = params_all.lookup("Fopr_TopologyFixing");
    ...
    const string str_fopr_prec_type = params_fopr_prec.get_string("fermion_type");
    //- NB. gamma_matrix_type of Wilson and Wilson_TwistedMass must be the same.
    ...
    //- preconditioner
    unique_ptr<Fopr> fopr_prec(Fopr::New(str_fopr_prec_type, params_fopr_prec));

    unique_ptr<Force> force_fopr_prec(new Force_F_Wilson_TwistedMass_Nf2(params_fopr_prec));

    unique_ptr<Fopr>  fopr_smear_prec(Fopr::New("Smeared", fopr_prec.get(), dr_smear.get()));
    unique_ptr<Force> force_fopr_smear_prec(new Force_F_Smeared(force_fopr_prec.get(), force_smear.get(), dr_smear.get()));


    unique_ptr<Solver> solver_H_prec(Solver::New(str_solver_H_type, fopr_smear_prec.get(), params_solver_H));
    unique_ptr<Fprop>  fprop_H_prec(new Fprop_Standard_lex(solver_H_prec.get()));


    unique_ptr<Action> action_F(new Action_F_Ratio_lex(fopr_smear_prec.get(), force_fopr_smear_prec.get(), fopr_smear.get(), force_fopr_smear.get(), fprop_H_prec.get(), fprop_MD.get(), fprop_H.get()));
