...
namespace Test_HMC_Domainwall {
  const std::string test_name = "HMC.Domainwall.Nf2_PV";

  //- test-private parameters
  namespace {
    const std::string filename_input = "test_HMC_Domainwall_Nf2_PV.yaml";
  }
  ...
  int update_Nf2_PV(void)
  {
    ...
    // Fopr *fopr_dw = Fopr::New("Domainwall", fopr_w, params_dw);
    ...
    //- Pauli-Villars
    // Fopr *fopr_dw_pv = Fopr::New("Domainwall", fopr_w, params_dw_pv);
    unique_ptr<Fopr> fopr_dw_pv(Fopr::New("Domainwall", params_dw_pv));

    unique_ptr<Force> force_fopr_dw_pv(new Force_F_Domainwall(params_dw_pv));

    unique_ptr<Fopr>  fopr_smear_pv(Fopr::New("Smeared", fopr_dw_pv.get(), dr_smear.get()));
    unique_ptr<Force> force_fopr_smear_pv(new Force_F_Smeared(force_fopr_dw_pv.get(), force_smear.get(), dr_smear.get()));


    unique_ptr<Solver> solver_H_pv(Solver::New(str_solver_H_type, fopr_smear_pv.get(), params_solver_H));
    unique_ptr<Fprop>  fprop_H_pv(new Fprop_Standard_lex(solver_H_pv.get()));


    unique_ptr<Action> action_F(new Action_F_Ratio_lex(fopr_smear_pv.get(), force_fopr_smear_pv.get(), fopr_smear.get(), force_fopr_smear.get(), fprop_H_pv.get(), fprop_MD.get(), fprop_H.get()));
